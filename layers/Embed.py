...
def forward(self, x, x_mark):
    ...
    # Get the value embedding with channel encoding
    x_value = self.value_embedding(x, channel_encoding=channel_encoding)
    x_pos = self.position_embedding(x)
    x_temp = self.temporal_embedding(x_mark)
    
    # Append the embeddings instead of adding them
    # This would change the dimension from (batch, seq_len, d_model) to (batch, seq_len, 3*d_model)
    x_combined = torch.cat([x_value, x_pos, x_temp], dim=-1)
    
    # You might want to project it back to d_model dimensions
    # x_combined = self.final_proj(x_combined)  # You would need to add this layer
